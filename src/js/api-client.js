...
const ASS_API_CONFIG = {
    ...
    ENDPOINTS: {
        ...
        CARD_STATS_BULK: '/api/card/stats/last/bulk',
...
class AssApiClient {
    ...
    static async getBulkCardStats(cardIds) {
        try {
            return await this.makeRequest(ASS_API_CONFIG.ENDPOINTS.CARD_STATS_BULK, {
                method: 'GET',
    ...
    static async submitCardStats(cardId, statsData) {
        try {
            // This would be implemented when the API supports submitting stats
            // For now, we'll just log that stats were collected
            console.log(`Stats collected for card ${cardId}:`, statsData);
            
            // TODO: Implement actual stats submission when API endpoint is available
            // return await this.makeRequest('/api/card/stats/submit', {
            //     method: 'POST',
            //     body: JSON.stringify({ card_id: cardId, ...statsData })
            // });
